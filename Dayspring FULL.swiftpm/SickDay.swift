        ...
        VStack{
            ...
            VStack{
                ...
                if(currentFeature == "Challenge"){
                    VStack{
                        Text(ChallengeManager.sickHeaders[currentChallengeNum]).foregroundColor(Color.init("SuperLightBlue")).font(.system(.largeTitle, design: .default)).bold()
                        Spacer().frame(height: screenWidth/20)
                        Text(ChallengeManager.sickInstructions[currentChallengeNum]).foregroundColor(Color.init("SuperLightBlue")).font(.system(.caption, design: .default)).bold().multilineTextAlignment(.center).bold()
                        Spacer().frame(height: screenWidth/20)
                        if(ChallengeManager.itemFound){
                            Button(action:{
           ...
           //STOPWATCH WORK
            ...
            ChallengeManager.currentChallenge = 4
        }
    }
}
